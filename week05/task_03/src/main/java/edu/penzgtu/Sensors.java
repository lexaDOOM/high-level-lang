...
public class Sensors {
    ...
    public void sensorDataProc(String[] readings) throws InvalidSensorDataException {
        for (String reading : readings) {
            if (reading.length() < 3) {
                throw new InvalidSensorDataException("Wrong data: " + reading);
            }
            String sensorId = reading.substring(0, 2);
            int temp;

            try {
                temp = Integer.parseInt(reading.substring(2));
